...
int UILoadDialog::getLvl() {
    namespace fs = std::filesystem;
    SDL_Window *m_window = nullptr;
    SDL_Renderer *m_renderer = nullptr;
    m_window = SDL_CreateWindow("Select level", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, SAVE_DIALOG_SCREEN_WIDTH,
                                SAVE_DIALOG_SCREEN_HEIGHT, 0);
    ...
    for (int i = 0; i < 2; ++i) {
        lvlsUI.push_back(UIButton({ 0,  buldingDx, controlHeight }, m_window, m_renderer, true));
    }
